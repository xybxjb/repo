...
$(function(){
	...
	function init(){
		$('#listData').datagrid({
	        url: 'proxyTeacherAuditing/waitToSubmit',//请求方法的地址
	        title: '查询结果',
	        ...
	        columns: [[
	            ...
	            {	
	            	field: 'opt', title: '操作', align: 'center',width: 80,formatter:opt
	            }
	        ]]
	    });
	}
	...
	function opt(value,row,index){
		 var str = "";
		    str += '<a href="javascript:void(0);" onclick="xiangqing('+row.id+')">报&nbsp;销</a>';
		    return str;
	}
	function search(){
	...
	function xiangqing(id){
		$.ajax({
			type : "post",//请求方式
			data:{"id":id},
			url : "proxyTeacherAuditing/getByIdWait",
			async : true,
	...
	function submit(){
		//判断参数
	      //发布文章的方法
	        var url = "";
	        var params = $("#xiangqingForm").serialize();
	        console.log(params);
	        //异步处理
	        $.post(url,params);
...
<body>
	...
	<div id="xiangqing_win" class="easyui-window" closed="true" style="position: relative; top: 10px;overflow: hidden" >
		<form id="xiangqingForm" action="" method="post" name="form1">
			<input id="id" type="hidden"  name="id" />
			<div style="text-align: center;margin-bottom: 10px">
				<label>姓&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;名：</label><input id="name" type="text" style="border: none;"readonly="readonly"/>
				<label>性&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;别：</label><input id="sex" type="text" style="border: none"readonly="readonly"/>
			</div>
			<div style="text-align: center;margin-bottom: 10px">
				<label>地&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;址：</label><input id="address" type="text" style="border: none" readonly="readonly"/>
				<label>电&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;话：</label><input id="tel" type="text" style="border: none"readonly="readonly"/>
			</div>			
			<div style="text-align: center;margin-bottom: 10px">
			...
			<div style="text-align: left;margin-bottom: 10px;width: 200px;margin-left: 28px">
				<label>来访目的：</label><input id="purpose" type="text" style="border: none;width: 60px" readonly="readonly"/>
			...
			<a ><input class="inp" type="button" onclick="document.getElementsByName('form1')[0].action='proxyTeacherAuditing/save2';document.getElementsByName('form1')[0].submit();" value="同意" style="position: relative;left: 180px;width:50px;height: 30px" /></a>
			<a><input class="inp" type="button" onclick="document.getElementsByName('form1')[0].action='proxyTeacherAuditing/save3';document.getElementsByName('form1')[0].submit()" value="不同意" style="position: relative;left: 220px;width:50px;height: 30px" /></a>
		</form>
	</div>
</body>
</html>