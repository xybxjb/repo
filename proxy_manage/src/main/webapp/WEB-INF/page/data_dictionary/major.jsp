...
<!--编辑页面 -->
	<div id="w" class="easyui-window" title="信息修改" data-options="iconCls:'icon-save'" style="width:320px;height:150px;padding:10px;" closed="false">
		<form  id="fromMajorDate">
			<input style="margin-top:5px" type="hidden" name="id" id="mId" >
			<div class="editwindow" style="padding-left:20px">专&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp业：<input style="margin-top:5px" class="easyui-textbox" name="name" id="mname"> </div>
			<div class="editwindow" style="padding-left:40px"><input type="button" value="保存" class="test" onclick="updateOrAddDate('fromMajorDate','update');"style="text-align:center;position: absolute;left:33%;width:100px;margin-top:15px;height:30px"></div>
		</form>
	</div>
	<div id="add_win" class="easyui-window" title="添加专业"  closed="true" style="width:275px;height:130px;padding:10px;">
		<form action="major/add" id="major_add_form">
			<div class="editwindow" style="padding-left:0px">专&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp业：<input type="text" id="major_name"  name="name" style="margin-top:5px" class="easyui-textbox"> </div>
			<div class="editwindow" style="padding-left:0px"><input type="button" value="保存" class="test" onclick="updateOrAddDate('major_add_form','add');"></div>
		</form>
	</div>
</body>
...
$(function () {
	...
	function init(){
		$('#listData').datagrid({
	        ...
	        columns: [[	        
	            {
	                field: 'name', title: '专业名称', width: 80, align: 'center'
	            },
	            {	
...
// 	编辑
	...
	function updateOrAddDate(form,method){
		...
		if(form=='major_add_form'){
			if($('#major_name').textbox('getValue')==''){
				alert("专业不能为空")
			}else{
				$.ajax({
					url: "major/"+method,    //请求的url地址
					...
					success: function(req){
						...
						$('#fromMajorDate').form('clear');
						$('#major_add_form').form('clear');
					}
			    });
			}
		}else{
