...
<!--编辑页面 -->
	<div id="w" class="easyui-window" title="信息修改" data-options="iconCls:'icon-save'" style="width:320px;height:150px;padding:10px;" closed="false">
		<form  id="fromEducationDate">
			...
			<div class="editwindow" style="padding-left:20px">出行方式：<input style="margin-top:5px" class="easyui-textbox" name="name" id="mname"> </div>
	...
	<div id="add_win" class="easyui-window" title="添加学历"  closed="true" style="width:275px;height:130px;padding:10px;">
		<form action="education/add" id="education_add_form">
			<div class="editwindow" style="padding-left:0px">出行方式：<input type="text" id="major_name"  name="name" style="margin-top:5px" class="easyui-textbox"> </div>
...
$(function () {
	...
	function init(){
		$('#listData').datagrid({
	        url: 'transportation/getAll',//请求方法的地址
	        title: '查询结果',
	        ...
	        columns: [[	        
	            {
	                field: 'name', title: '出行方式', width: 80, align: 'center'
	            },
	            {	
	...
	function updateOrAddDate(form,method){
		...
		if(method=='add'){
			if($('#major_name').textbox('getValue')==''){
				...
				$.ajax({
					url: "transportation/"+method,    //请求的url地址
