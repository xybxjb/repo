...
function init(){
	$('#listData').datagrid({
        url: 'unaudited/list?id=0',//请求方法的地址
        title: '查询结果',
        height: 500, 
...
function opt(value,row,index){
	var str='<a href="javascript:void(0);" onclick="check('+row.student.id+');">审核</a>';
	return str;
}
...
function pass(val){
	...
	var fields = $('#checkForm').serialize()+"&auditStatus="+val;//获取checkForm中的数据并序列化(转换成字符串)
	$.ajax({
		type:"post",//请求方法
		...
		success:function(data){
			if(data=="用户请先登录"){
				...
				$('#checkWin').window('close');				//关闭弹框
				$('#listData').datagrid('reload');				//保存后刷新table表格获取或删除数据
				$('#checkWin').form('clear');				//清楚form中的数据
			}
		}
	})
}
...
function search(){
	...
	$('#listData').datagrid({"url":"unaudited/list?id=0&"+date});
}

function clean(){
	$('#searchForm').form('clear');
	$('#listData').datagrid({'url':'unaudited/list?id=0'});
	}
</script>
...
<body>
	<%/*
		模糊查询
			根据学生的姓名、班级、地址和来校时间对学生的来校报销进行模糊查询
	*/ %>
	<div>
...
<div id="checkWin" class="easyui-window"  closed="true" style="width:600px;height:500px" >
	<form id="checkForm" action="">
		...
		<div>
			<label>&nbsp;&nbsp;&nbsp;审核时间：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label><input type="text" id="date" name="auditTime" class="easyui-datetimebox"> 
		...
		<div style="text-align:center; margin-top:40px; margin-bottom:5px">
			<input type="button" value="通过" onclick="pass(2)"  style="width:60px;height:30px"   class="easyui-linkbutton">
			<input type="button" value="不通过" onclick="pass(1)" style="width:60px;height:30px"   class="easyui-linkbutton">
